...
const response = await client.search({ q: 'REPLACE_ME' }).asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: result, response: raw } = await client.search({ q: 'REPLACE_ME' }).withResponse();
...
1. Via the `MCPTEST_LOG` environment variable
...
const client = new Mcptest({
  logger: logger.child({ name: 'Mcptest' }),
...
const client = new Mcptest({ fetch });
```
...
```ts
import Mcptest from 'npm:mcptest';
